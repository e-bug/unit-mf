...
for (k, init_lr, lambda_u, lambda_i) in hyperparams:
    ...
    cv = CvMF(algorithm, num_features=k, nanvalue=nanvalue, lr0=init_lr,
              decay_fn=lambda lr, step: inverse_time_decay(lr, step, decay_rate, max_epochs, False),
              lambda_user=lambda_u, lambda_item=lambda_i, max_epochs=max_epochs, int_iter=int_iter, err_fn=err_fn)
